...
1. 使用 Llama-3.1-8B-Instruct 作为主模型 (资源紧张可更换 Llama3 系列的 3B/1B 等更小的模型，应该也能work)。
2. 实现或接入固定预算 SnapKV。
...
在完成基础复现后，需要分析本方法在实际长上下文推理中的局限，并提出改进思路。分析应尽量结合实验现象，而不是只给概念性描述。可重点讨论以下问题：

1. **注意力熵只是间接指标**：高熵 attention head 不一定总是更重要，低熵 head 也可能关注关键证据 token。仅用熵分配预算可能误判语义重要性。
2. **预算分配依赖固定超参数**：`alpha`、`beta`、`H_bar`、观察窗口长度等设置会影响结果，不同任务、不同上下文长度下未必都适用。
3. **动态预算存在额外开销**：EntroKV 需要读取 attention weights、计算熵并执行 per-head top-k，可能增加 prefill latency。
4. **对生成阶段变化适应不足**：如果预算主要由 prefill 阶段的观察窗口决定，后续 decode 阶段 attention 模式变化时，已压缩的 KV cache 难以及时恢复。
5. **极端预算分配风险**：部分 head 预算过低时可能丢失必要上下文，部分 head 预算过高时又会削弱整体压缩收益。

在此基础上，至少提出一种改进方案。算力允许时，建议实现其中一种并做小规模实验验证，例如：

1. **预算上下限约束**：为每个 layer-head 设置 `B_min` 和 `B_max`，避免某些 head 被过度压缩或过度分配，然后重新归一化预算总量。
2. **熵与注意力质量混合打分**：将归一化熵与 attention top-k mass、recent token 权重或任务相关 token 权重结合，减少只依赖熵的误判。
3. **layer-aware 分配**：为不同层设置不同 `alpha` 或预算缩放系数，观察浅层、中层、深层对压缩的敏感性差异。
4. **动态超参数消融**：比较不同 `alpha`、`beta`、观察窗口长度和预算上下限设置，分析改进是否稳定。

若进行了实现和实验验证，报告中应至少包含：

1. 改进方法的公式或伪代码。
2. 与固定预算 SnapKV、原始 EntroKV 的对比表。
3. 至少一个任务或少量样本上的分数、平均预算、显存或延迟变化。
4. 对结果的判断：改进是否真的有效，在哪些任务上有效，代价是什么。
...
9. **方法不足与改进**：分析本方法的局限，提出改进思路；若已实现改进，需要给出实验设置、结果和结论。
